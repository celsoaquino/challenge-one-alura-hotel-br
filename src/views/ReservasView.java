...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblValorSimbolo.setBounds(78, 332, 28, 25);
		lblValorSimbolo.setForeground(SystemColor.textHighlight);
		lblValorSimbolo.setFont(new Font("Roboto", Font.BOLD, 18));
		...
		lblCheckOut.setBounds(68, 221, 196, 14);
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
					...
					txtValor.setText(updateValue(txtDataE, txtDataS));
				}
			}
		});
		...
		txtValor.setBounds(111, 328, 93, 33);
		...
		txtFormaPagamento.setModel(new DefaultComboBoxModel<String>(
				new String[] { "Selecione...", "Cartão de Crédito", "Cartão de Débito", "Dinheiro" }));
		panel.add(txtFormaPagamento);

		JLabel lblFormaPago = new JLabel("FORMA DE PAGAMENTO");
		lblFormaPago.setForeground(SystemColor.textInactiveText);
		lblFormaPago.setBounds(68, 382, 243, 24);
		lblFormaPago.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblFormaPago);

		JLabel lblTitulo = new JLabel("SISTEMA DE RESERVAS");
		lblTitulo.setBounds(109, 60, 259, 42);
	...
	private void save() {
		LocalDate entrada = ConverterDate.dateToLocalDate(txtDataE.getDate());
		LocalDate saida = ConverterDate.dateToLocalDate(txtDataS.getDate());
		BigDecimal valor = calculateReservationAmount(entrada, saida);
		String formaPagamento = txtFormaPagamento.getSelectedItem().toString();

		Reserva reserva = new Reserva(entrada, saida, valor, formaPagamento);
		reserva = this.reservaController.save(reserva);
		idReserva = reserva.getReservaId();
	}

	private boolean isValidFileds() {
		if (txtDataE.getDate() == null || txtDataS.getDate() == null || txtValor.getText().equals("")
				|| txtFormaPagamento.getSelectedIndex() == 0) {
			return false;
		}
		return true;
	}

	private BigDecimal calculateReservationAmount(LocalDate entrada, LocalDate saida) {
		long diferencaEmDias = ChronoUnit.DAYS.between(entrada, saida);
		BigDecimal result = Reserva.DIARIA.multiply(new BigDecimal(diferencaEmDias));
		result = result.setScale(2, RoundingMode.HALF_EVEN);
		return result;
	}

	private String updateValue(JDateChooser entrada, JDateChooser saida) {
		LocalDate localDateEntrada = ConverterDate.dateToLocalDate(entrada.getDate());
		LocalDate localDateSaida = ConverterDate.dateToLocalDate(saida.getDate());
		BigDecimal value = calculateReservationAmount(localDateEntrada, localDateSaida);
		String parsedValue = new DecimalFormat(" #,##0.00").format(value);
		return parsedValue;
	}
}
